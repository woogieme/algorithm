...
public class 배열돌리기4 {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < T; i++) {
			...
			pnt.add(new Point(arr[0]-arr[2]-1,arr[1]-arr[2]-1,arr[0]+arr[2],arr[1]+arr[2]));
		}
	...
	private static void rotate(int i) {
		...
		for (int d = 0; d < depth; d++) {
			...
			while(true) {
				...
				if(nexti==N_real-d || nextj==M_real-d || nexti==(pnt.get(i).idx_i)+d-1 ||nextj==(pnt.get(i).idx_j)+d-1)
				{
					dir+=1;
					if(dir==4) {
						break;
