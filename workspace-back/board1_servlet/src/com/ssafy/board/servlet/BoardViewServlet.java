...
//		2. 1의 글번호를 이용하여 Dao의 viewArticle()메소드에 전달하고 DB에서 select 하세요.
		...
		BoardDto list =dao.viewArticle(Integer.parseInt(articleNo));
...
//		3. 2에서 얻어온 글정보를 출력하세요.
		...
		out.println("            <h2 class=\"text-secondary px-5\">"+list.getSubject()+"</h2>");
		out.println("          </div>");
		...
		out.println("                />");
		out.println("                <p>");
		out.println("                  <span class=\"fw-bold\">"+list.getUserId()+"</span> <br />");
