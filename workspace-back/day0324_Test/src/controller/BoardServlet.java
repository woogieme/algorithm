	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			if (action == null || action.equals("list")) {
				...
				int page =1;
				if(pageParam!=null && pageParam.length()>0) { //page=hello 이런거 체크도 사실 해야됨 
					page=Integer.parseInt(pageParam);	
				}
				Map<String, Object> boardPage = service.getBoards(page);
...
//				resp.sendRedirect("BoardWrite.jsp");
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			} else if(action.equals("deleteBoards")) {
				String[] bnos = req.getParameterValues("bnos");
				int result = service.deleteBoards(bnos);
				req.setAttribute("deleteResult", result);
				req.getRequestDispatcher("BoardDeleteSuccess.jsp").forward(req, resp);
			}
		} catch (SQLException e) {
			e.printStackTrace();
			req.setAttribute("errorMsg", "게시글 작성중 오류 발생. 값 확인 요망");
			req.getRequestDispatcher("Error.jsp").forward(req, resp);
		}
	}
}
