...
//	}
	...
	@PostMapping("/write")
	public String write(BoardDTO board,MultipartFile[] uploadedFiles, HttpSession session) throws SQLException, IllegalStateException, IOException { 
		...
		if(loginInfo == null) { // 로그인 안된 사용자는 글쓰기 화면 안줌.
			...
			service.write(board, uploadedFiles); // 지금 작성하는 게시글 번호를 파일에 기록해서 FILE_TB 넣어야함.
	...
	public ModelAndView read(int bno) throws SQLException {
		...
		mv.addObject("board", service.read(bno)); // boardDTO에 이제 files도 들어가있음.
		return mv;
	}
